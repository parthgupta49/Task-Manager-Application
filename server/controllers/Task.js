...
exports.addTask = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { $push: { tasks: task._id } }, { new: true }).populate("tasks");
...
exports.editTask = async (req, res) => {
    try {
        ...
        if (!updatedTask) {
            return res.status(400).json({ success: false, message: "Couldn't update the task" });
        }
...
exports.deleteTask = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, { $pull: { tasks: taskId }, }, { new: true }).populate("tasks");
        if (!updatedUser) {
            return res.status(400).json({
